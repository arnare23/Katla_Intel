...
// .wrangler/tmp/bundle-oyr4t9/checked-fetch.js
...
async function signJWT(payload, secret) {
  ...
  return new SignJWT(payload).setProtectedHeader({ alg: ALGORITHM }).setIssuedAt().setExpirationTime(EXPIRY).setIssuer("katla-api").sign(key);
}
...
publicPosts.get("/:slug/related", async (c) => {
  ...
  ).bind(current.category, slug, "published").all();
  return c.json({ data: (results || []).map(parsePostRow) });
});
var adminPosts = new Hono2();
...
adminPosts.post("/", async (c) => {
  ...
  const publishedAt = body.status === "published" ? body.published_at || now : body.published_at || null;
...
adminPosts.put("/:id", async (c) => {
  ...
  let publishedAt = body.published_at !== void 0 ? body.published_at : existing.published_at;
  if (body.status === "published" && existing.status !== "published" && !publishedAt) {
    publishedAt = now;
  }
  await c.env.DB.prepare(
    `UPDATE posts SET
    ...
    body.author !== void 0 ? body.author : existing.author,
    body.excerpt !== void 0 ? body.excerpt : existing.excerpt,
    body.content !== void 0 ? body.content : existing.content,
    body.featured_image !== void 0 ? body.featured_image : existing.featured_image,
    body.tags !== void 0 ? JSON.stringify(body.tags) : existing.tags,
    body.read_time !== void 0 ? body.read_time : existing.read_time,
    body.status ?? existing.status,
    body.featured !== void 0 ? body.featured ? 1 : 0 : existing.featured,
    publishedAt,
    now,
    id
  ).run();
...
__name(parsePostRow, "parsePostRow");
...
adminEnquiries.get("/:id", async (c) => {
  const id = c.req.param("id");
  const row = await c.env.DB.prepare("SELECT * FROM enquiries WHERE id = ?").bind(id).first();
  if (!row) {
...
// src/routes/case-studies.ts
var publicCaseStudies = new Hono2();
...
var adminCaseStudies = new Hono2();
...
adminCaseStudies.put("/:id", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    body.slug ?? existing.slug,
    body.category !== void 0 ? body.category : existing.category,
    body.client !== void 0 ? body.client : existing.client,
    body.description !== void 0 ? body.description : existing.description,
    body.content !== void 0 ? body.content : existing.content,
    body.featured_image !== void 0 ? body.featured_image : existing.featured_image,
    body.metrics !== void 0 ? JSON.stringify(body.metrics) : existing.metrics,
    body.technologies !== void 0 ? JSON.stringify(body.technologies) : existing.technologies,
    body.status ?? existing.status,
    body.featured !== void 0 ? body.featured ? 1 : 0 : existing.featured,
    body.display_order !== void 0 ? body.display_order : existing.display_order,
    now,
    id
  ).run();
  return c.json({ success: true });
...
function parseCaseStudyRow(row) {
  return {
    ...row,
    metrics: safeJsonParse2(row.metrics, []),
    technologies: safeJsonParse2(row.technologies, []),
    featured: row.featured === 1
  };
}
__name(parseCaseStudyRow, "parseCaseStudyRow");
...
adminJobs.put("/:id", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    body.title ?? existing.title,
    body.type !== void 0 ? body.type : existing.type,
    body.location !== void 0 ? body.location : existing.location,
    body.department !== void 0 ? body.department : existing.department,
    body.short_description !== void 0 ? body.short_description : existing.short_description,
    body.requirements !== void 0 ? JSON.stringify(body.requirements) : existing.requirements,
    body.content !== void 0 ? body.content : existing.content,
    body.status ?? existing.status,
    body.display_order !== void 0 ? body.display_order : existing.display_order,
    now,
...
function parseJobRow(row) {
  return {
    ...row,
    requirements: safeJsonParse3(row.requirements, [])
  };
}
__name(parseJobRow, "parseJobRow");
function safeJsonParse3(value, fallback) {
  if (!value) return fallback;
  try {
    return JSON.parse(value);
  } catch {
...
adminResearch.put("/:id", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    body.title ?? existing.title,
    body.slug ?? existing.slug,
    body.authors !== void 0 ? JSON.stringify(body.authors) : existing.authors,
    body.abstract !== void 0 ? body.abstract : existing.abstract,
    body.content !== void 0 ? body.content : existing.content,
    body.featured_image !== void 0 ? body.featured_image : existing.featured_image,
    body.tags !== void 0 ? JSON.stringify(body.tags) : existing.tags,
    body.pdf_url !== void 0 ? body.pdf_url : existing.pdf_url,
    body.status ?? existing.status,
    publishedAt,
    now,
    id
...
function parseResearchRow(row) {
  return {
    ...row,
    authors: safeJsonParse4(row.authors, []),
    tags: safeJsonParse4(row.tags, [])
  };
}
__name(parseResearchRow, "parseResearchRow");
function safeJsonParse4(value, fallback) {
  if (!value) return fallback;
  try {
    return JSON.parse(value);
  } catch {
...
var publicSubscribers = new Hono2();
...
var adminSubscribers = new Hono2();
...
// src/routes/stats.ts
var adminStats = new Hono2();
adminStats.use("*", requireAuth);
adminStats.get("/", async (c) => {
  const [enquiriesResult, postsResult, caseStudiesResult, jobsResult] = await Promise.all([
    c.env.DB.prepare(
...
// .wrangler/tmp/bundle-oyr4t9/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-oyr4t9/middleware-loader.entry.ts
